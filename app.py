...
import cv2
import torch
from deeplearning import plate_detection_i, character_localize_i, character_recognition_i
...
#----------SQLAlchemy-----------------------
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:@localhost/hi_db'
app.config['SQLALCHEMY_MODIFICATIONS'] = True
db = SQLAlchemy(app)


class Parkingdata_videos(db.Model):
...
# app.config['MYSQL_DB'] = 'cap_db'
#
# mysql1 = MySQL(app)
# cursor1 = mysql1.connection.cursor()
...
# def save_data(prediction):
#     cursor1 = mysql1.connection.cursor()
#     cursor1.execute(''' INSERT INTO parking_lot VALUES(%s,%s,%s)''', (prediction, str(date.today()), datetime.now()))
...
def Img():
    if request.method == 'POST':
        ...
        # upload_file1 = cv2.resize(upload_file, (500, 400))
        filename = upload_file.filename
        path_save = os.path.join(UPLOAD_PATH, filename)
        upload_file.save(path_save)
        only_plate = plate_detection_i(path_save, filename)
        localize_character = character_localize_i(only_plate, filename)
        prediction = character_recognition_i(localize_character)
        print(prediction)

        # save_data(prediction)
        # save--------------------------------------------

        entry = Parkingdata_videos(plate_number=prediction, date=date.today(),
                                                                  time=datetime.now())
        ...
        return render_template('img.html',
                               upload=True, filename=filename,
                               prediction=prediction
                               )

    return render_template('img.html', upload=False)
...
def character_localize(after_validation_img, filename):
    with app.app_context():
                            ...
                            for *xyxy, conf, cls in reversed(det):
                                ...
                                if conf > 0.90 and cnt < 1:
                                    ...
                                    if (ar > 1.3 and ar <= 2.6):
                                        ...
                                        prediction = character_recognition(loc_img)
                                        print(prediction)
                                        ...
                                        # ---------mysql--------------------------
                                        # cursor1 = mysql1.connection.cursor()
                                        # cursor1.execute(''' INSERT INTO parkingdata_video VALUES(%s,%s,%s)''',
                                        #                (prediction, str(date.today()), datetime.now()))
                                        # mysql1.connection.commit()
                                        # cursor1.close()
...
@app.route('/web')
def webcam():
    data = db.session.query(Parkingdata_videos).filter(Parkingdata_videos.date==date.today())
    return render_template('frame.html', upload=False, data=data)

@app.route('/frame&data')
def Frame_Data():
    data = db.session.query(Parkingdata_videos).filter(Parkingdata_videos.date==date.today())
    return render_template('video.html', upload=False, data=data)


@app.route('/video', methods=['POST','GET'])
def video():
    return Response(detect_from_video(),mimetype='multipart/x-mixed-replace; boundary=frame')
...
@app.route('/data')
def DataPage():
    data = db.session.query(Parkingdata_videos)
    return render_template('data_page.html', upload=False, data=data)
# if __name__ == "__main__":  .order_by(Parkingdata_video.date.desc())
#     app.run(debug=True)
